...
subtitle: "Manipulación básica de objetos"
author: "Pablo Cabrera-Álvarez (USAL)"
date: "Madrid, 15 de junio de 2019"
output:
...
### 4. ¿Y todo esto para hacer una tabla?
...
### - Reproducir $\to$ Practicar $\to$ Aplicar
...
- El caso especial de los **factores** o cómo trabajar con datos de encuesta
     
- **Operadores**: `+`, `-`, `/`, `*`, `**`,  COMPLETAR
    
- **Indexación** en `R`: `[]`, `[[]]`, `$`

- El poder de **transformar objetos**: `class()` y `as._()`
     
- Cadenas de **texto**: funciones básicas
     
- **Fechas**: funciones básicas 
...
`File` $\to$ `New projecjt...` $\to$ `New directory`

La segunda opción consiste en fijar manualmente el directorio de trabajo:
...
vector_double <- c(1.87, 2.4, 5.8754)
vector_double
```
...
#### `data.frame(..., stringsAsFactors = TRUE)` | `rnorm(n, mean, sd)` | `runif(n, min, max)`

Los **data frames** son conjuntos de datos en los que las columnas pueden ser de diferente tipo. Equiparable al conjunto de datos estándar utilizado en las Ciencias Sociales, casos en las filas y variables en las columnas.
...
my_list <- list(my_vector, my_df) # generar lista
my_list
```
...
Algunas funciones vienen por defecto en R (8 paquetes), pero otras se encuentran en paquetes creados por autores. Los paquetes se encuentran disponibles en un repositiorio llamado **CRAN**.
...
# Factores: trabajar con datos de encuestas
...
Una de las operaciones básicas con los factores es cambiar las etiquetas, para ello se utiliza la función `recode()`. En este ejemplo se modifican `"Desemp."` $\to$ `"Desempleado"` y `"Retirado"` $\to$ `"Jubilado"`:
...
Los niveles de un factor se pueden reordenar con la función `fct_relevel()`. Por ejemplo, asociar el nivel `"Trabaja"` con el valor `1`. Los niveles no explicitados en la función son colocados al final en el mismo orden que estaban: 
...
--
`r emo::ji("red_circle")` Tras cargar un conjunto de datos una varaible de tipo numérico, como puede ser edad está almacenada como texto. El objetivo es calcular la media.
...
`r emo::ji("green_circle")` Transformar el vector en *numeric*:
...
`r emo::ji("red_circle")` R trata los factores de forma específica en algunos procedimientos, como por ejemplo en las regresiones `lm()`. Las variables de tipo factor son dicotomizadas, sin embargo, en muchas ocasiones al cargar el conjunto de datos la variable categórica está alamacenada como *character*.
...
`r emo::ji("green_circle")` Transformar la variable en factor usando `as.factor()`:
