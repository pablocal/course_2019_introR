...
Un **vector** es el objeto básico de `R`, un conjunto de números o caracteres. Todos los elementos d eun vector deben de ser del mismo tipo. 
...
my_list <- list(my_vector, my_matrix, my_df) # generar lista
my_list
```
...
df$varl
class(df$varl) #función: evaluar la clase del objeto
levels(df$varl) #función: evaluar los niveles del factor
```
