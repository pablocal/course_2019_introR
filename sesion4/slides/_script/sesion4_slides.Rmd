---
title: '¿Y tood esto para una tabla?'
...
- La función `frq()` cuenta con argumentos de tipo **estadístico,** como la inclusión de pesos (`weights`). Sin embargo, la mayoría de argumentos tienen que ver con el **formato** de la tabla. El argumento `sort.frq` permite ordenar las categorías de forma ascendente (`"asc"`) o descendente (`"desc"`). El argumento `show.na` acepta `TRUE` y `FALSE` y permite eliminar la visualización de la categoría `NA`.
...
#### `tab_cells(vars)` | `tab_stat_cases()` | `tab_pivot()`
...
- La función `tab_pivot()` indica que la tabla debe ser creada
