...
## D. Repaso general a la manipulación de datos

**D.1** Carga el data frame `autoesc_jun_2017.csv` y asignalo al nombre `auto` a continuación:
...
3. Calcula el total de presentados (`total_pres`), que será la suma de aptos y no aptos.
...
7. Cambiar el formato del data frame de ancho a largo, de forma que las variables `p_aptos_conv*` queden en dos columnas, la primera `convocatoria`, y la segunda `p_aptos`.
